...
- **해결 2:** 특정 플랫폼을 명시하여 빌드하십시오.
  `./scripts/build-specific-platform.sh v1.5.0 linux/amd64 yourusername`
...
| Job | 역할 | 트리거 |
|---|---|---|
| `rescaper-ci` | 의존성 설치 → 구문 체크 → Playwright E2E 테스트 (27개) | Push 시 자동 |
...
```bash
# 특정 버전으로 배포
RESCAPER_TAG=v1.5.0 docker compose -f docker/docker-compose.prod.yml pull
RESCAPER_TAG=v1.5.0 docker compose -f docker/docker-compose.prod.yml up -d
