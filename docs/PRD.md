...
| 항목 | 값 |
|---|---|
| 문서 버전 | v8.0 (Mobile Browser Support) |
| 기준일 | 2026-03-26 |
| 프로젝트명 | RescapeR |
| 목표 플랫폼 | Web (PWA), Desktop (Docker), Mobile Browser (터치 가상 패드) |
...
- **시네마틱 연출:** 오프닝(게임 시작 시)/엔딩(9층 클리어 후) 랜덤 멘트를 타이핑 효과 + 페이드아웃으로 표시. 공통 함수 `showCinematic` 사용.
- **모바일 브라우저 지원:** 터치 가상 패드(touch-system.js)를 통한 모바일 플레이. 자동 모바일 감지, 멀티터치 지원, 오버레이 UI 터치/클릭 대응. 반응형 CSS로 모바일 시인성 최적화.
