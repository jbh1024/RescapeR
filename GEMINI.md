# GEMINI.md - RescapeR AI Agent Context & Guide

## 🚀 프로젝트 개요
**RescapeR**는 "퇴근을 위해 회사라는 던전을 탈출하라!"를 테마로 한 2D 로그라이트 액션 플랫폼어 게임입니다.  
본 가이드는 AI 에이전트가 코드 수정, 기능 추가, 리팩토링 시 준수해야 할 핵심 지침을 담고 있습니다.

## 🏗️ 하이브리드 아키텍처 이해
1.  **Playable Web (주력):** HTML5 Canvas + Vanilla JavaScript (ES6 Modules). **현재 개발 및 배포의 핵심 타겟입니다.**
2.  **Ranking Backend (신설):** Node.js + Express + SQLite. 글로벌 랭킹 기록 및 보안 검증(HMAC)을 담당합니다.
3.  **Unity Scaffold (참조):** 향후 Unity 이식이나 구조적 참조를 위한 C# 스크립트.

## 📘 핵심 참조 문서
- **기능 요구사항 & 아키텍처:** `docs/PRD.md` (Product Requirement Document)
- **게임 디자인 & 상세 데이터:** `docs/GDD.md` (Game Design Document)
- **랭킹 시스템 명세:** `docs/RANKING_SYSTEM.md`
- **정적 데이터 상수:** `playable-web/systems/data-config.js`

## 🛠️ 에이전트 개발 규칙 (Conventions)

### 1. 코드 수정 및 밸런스 원칙
- **우선순위:** 신규 기능 및 버그 수정은 기본적으로 `playable-web/`에 먼저 적용합니다.
- **모듈화:** 로직은 `playable-web/systems/` 내의 적절한 모듈에 캡슐화합니다.
- **상태 관리:** 모든 런타임 데이터는 `state` 객체에서 관리합니다. 전역 변수 사용을 지양하세요.
- **재시작 로직:** 매 게임 시작(`startRun`) 시 `localStorage.clear()`를 통해 로컬 데이터를 초기화합니다.
- **시네마틱 시스템:** 오프닝(`OPENING_LINES`)/엔딩(`ENDING_LINES`) 멘트 배열에서 랜덤 1개를 타이핑 효과로 표시 후 페이드아웃. 공통 함수 `showCinematic(lines, onComplete)` 사용.
- **랭킹 보안**: 모든 기록 제출은 `HMAC-SHA256` 체크섬 검증을 거쳐야 하며, 최소 클리어 타임(30초) 미만은 서버에서 차단합니다.
- **오프라인 대응**: 서버 연결 실패 시 `localStorage`에 기록을 버퍼링하고 다음 시작 시 재전송합니다.

### 2. 스타일 및 네이밍
- **언어:** UI와 문서는 한국어 위주, 코드와 주석은 기존 스타일을 따릅니다.
- **시스템 모듈:** `PascalCase` 사용 (예: `RenderSystem`, `RankingSystem`).
- **용어 준수**: 모든 재화는 **'야근수당'**, 랭킹 보드는 **'명예의 퇴근 명부'** 명칭을 고수합니다.

### 3. 검증 및 보안 프로세스 (필수)
- **JS 난독화:** 프로덕션 빌드 시 `javascript-obfuscator`를 적용합니다.
- **자동화 테스트:** `npx playwright test` (신규 기능 및 랭킹 시스템 검증 포함)

## 🕹️ 조작 표준 (Key Bindings)
- 이동: `← / →` | 점프: `↑` | 공격: `Space` | 대시: `Shift`
- 상호작용/상점/이벤트: `E` | 소비 슬롯: `Q`
- 스킬 선택: `1 / 2 / 3`
- **랭킹 리더보드: `K`** (토글 기능, 열릴 시 게임 일시정지)
- **정보 패널 토글: `H`** (로그와 조작법을 3:2 좌우 분할 레이아웃으로 표시)
- 일시정지: `P` | 재시작(사망): `R`
- **이스터에그 (치트)**: **정보 패널(H)이 ON 상태일 때만 작동**
  - 다음 층 이동: `Ctrl + ]`
  - 이전 층 이동: `Ctrl + [`
  - 야근수당 +100: `Ctrl + 0`
